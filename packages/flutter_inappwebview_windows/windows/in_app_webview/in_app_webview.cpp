...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebView::registerSurfaceEventHandlers()
  {
    ...
    // Register for focus events on the controller
    if (webViewController) {
      failedLog(webViewController->add_GotFocus(
        Callback<ICoreWebView2FocusChangedEventHandler>(
          [this](ICoreWebView2Controller* sender,
            IUnknown* args) -> HRESULT
          {
            if (focusChangedCallback_) {
              focusChangedCallback_(true);
            }
            return S_OK;
          })
        .Get(), nullptr));

      failedLog(webViewController->add_LostFocus(
        Callback<ICoreWebView2FocusChangedEventHandler>(
          [this](ICoreWebView2Controller* sender,
            IUnknown* args) -> HRESULT
          {
            if (focusChangedCallback_) {
              focusChangedCallback_(false);
            }
            return S_OK;
          })
        .Get(), nullptr));
  ...
  void InAppWebView::moveFocus(bool focused)
  {
    if (!webViewController) {
      return;
    }
